...
bool GetMetricGpuValue(std::string chipName,
  ...
  for (size_t metricIndex = 0; metricIndex < metricNames.size();
    ...
    for (size_t rangeIndex = 0; rangeIndex < getNumRangesParams.numRanges;
      ...
      RETURN_IF_NVPW_ERROR(false, NVPW_MetricsEvaluator_SetDeviceAttributes(
      ...
      RETURN_IF_NVPW_ERROR(false, NVPW_MetricsEvaluator_EvaluateToGpuValues(
                                      &evaluateToGpuValuesParams));
      metricNameValue.rangeNameMetricValueMap.push_back(
          std::make_pair(rangeName, metricValue));
    }
...
bool PrintMetricValues(std::string chipName,
  ...
  std::cout << "\n"
            << std::setw(40) << std::left << "Range Name" << std::setw(100)
            << std::left << "Metric Name"
